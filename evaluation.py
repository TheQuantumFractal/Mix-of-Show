    ...
    for i in image_paths:
        if ">" in i:
            for j in ref_image_paths:
                try:
                    d = DeepFace.verify(i, j, model_name="ArcFace", enforce_detection=True, detector_backend="retinaface")
                    scores += (d["distance"] <= 0.68)
                    count += 1
                    # print(d["distance"])
                except:
                    print("Failed")
    print(f"scores for {args.image_dir}", scores / count, scores / (len(image_paths) * len(ref_image_paths)))
